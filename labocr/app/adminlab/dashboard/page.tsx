...
export default function DashboardPage() {
  ...
  // [P-05] Guard: cegah concurrent polling request
  ...
  const fetchStatus = useCallback(async () => {
    // [P-05] Cegah concurrent request: jika fetch sebelumnya belum selesai, skip
  ...
  // [P-03] Gunakan `peminjaman` (bukan labsWithUser.size) sebagai dependency
  //        agar useMemo benar-benar di-skip saat data tidak berubah
  const stats = useMemo(() => [
    {
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="relative bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col md:flex-row">
            ...
            <div className="w-full md:w-5/12 bg-[#0F172A] p-10 flex flex-col items-center justify-center">
              ...
              <div className="w-full space-y-4 border-t border-slate-800 pt-5 text-[10px]">
                <div className="flex justify-between items-start gap-4">
                  <span className="text-slate-500 font-bold shrink-0">Nama</span>
                  <span className="text-white font-bold text-right break-words">{selectedPeminjaman.nama}</span>
                </div>
                <div className="flex justify-between items-start gap-4">
                  <span className="text-slate-500 font-bold shrink-0">NIM</span>
                  <span className="text-white font-black text-right break-words">{selectedPeminjaman.nim}</span>
                </div>
                <div className="flex justify-between items-start gap-4">
                  <span className="text-slate-500 font-bold shrink-0">Lab</span>
                  <span className="text-emerald-400 font-black text-right break-words">{selectedPeminjaman.lab}</span>
                </div>
                <div className="flex justify-between items-start gap-4">
                  <span className="text-slate-500 font-bold shrink-0">Masuk</span>
                  <span className="text-white font-bold text-right break-words">{selectedPeminjaman.waktu_masuk?.slice(11, 16)}</span>
                </div>
              </div>
            </div>
