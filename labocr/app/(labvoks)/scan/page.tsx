...
const ScanLabPage = () => {
  ...
  // [F-08] Simpan stream di useRef untuk menghindari stale closure di cleanup
  ...
  const startCamera = useCallback(async () => {
    // Hentikan stream lama via ref (selalu current)
  ...
  const stopCamera = useCallback(() => {
    // [F-08] Akses ref langsung — tidak ada stale closure
    streamRef.current?.getTracks().forEach((track) => track.stop());
    streamRef.current = null;
    if (videoRef.current) videoRef.current.srcObject = null;
  }, []);
  ...
  useEffect(() => {
    ...
    // Cleanup: hentikan kamera saat unmount atau saat step/facing berubah
    return () => {
      streamRef.current?.getTracks().forEach((track) => track.stop());
      streamRef.current = null;
    };
  }, [currentStep, facingMode, startCamera, stopCamera]);
  // ─── API: Scan KTM ──────────────────────────────────
  const handleScanKTM = async () => {
    if (!videoRef.current) return;

    // Cek login sebelum lanjut proses scan
    const token = typeof window !== "undefined"
      ? localStorage.getItem("smartlab_jwt_token")
      : null;
    ...
    if (!token) {
      Swal.fire({
        icon: "warning",
        title: "Belum Login",
        text: "Anda harus login terlebih dahulu untuk menggunakan fitur scan.",
  ...
  const handleFaceAction = async () => {
    ...
    // Token pasti sudah ada karena sudah dicek di step 1
    const token = typeof window !== "undefined"
      ? localStorage.getItem("smartlab_jwt_token")
      : null;
  ...
  // ─── Formatted Date (dihitung dari waktu_masuk backend, bukan waktu mount) ──
  const getFormattedCheckinTime = () => {
    if (checkinData?.waktu_masuk) {
      // Gunakan waktu yang dikembalikan backend
      const d = new Date(checkinData.waktu_masuk);
      if (!isNaN(d.getTime())) {
        return {
    ...
    // Fallback: waktu sekarang (hanya jika backend tidak kirim waktu_masuk)
    const now = new Date();
    return {
                  ...
                  <div className="p-10 space-y-8">
                    ...
                    <div className="grid grid-cols-2 gap-y-8 pt-6 border-t border-slate-50">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <p className="text-sm font-black text-slate-700">{getFormattedCheckinTime().date}</p>
                      </div>
                      <div className="space-y-1.5 text-right">
